...
use wglymr_runtime::gpu::SurfaceHandle;
use wglymr_runtime::logging::RuntimeLogger;
use wglymr_runtime::EditorRuntime;
...
#[wasm_bindgen]
pub fn init_engine() {
    wglymr_runtime::logging::set_logger(&CONSOLE_LOGGER);

    EditorRuntime::with(|rt| {
        if let Err(e) = rt.init_engine() {
            wglymr_runtime::logging::error(&format!("Failed to initialize engine: {}", e));
        }
    });
}
...
pub async fn init_gpu() -> Result<(), JsValue> {
    ...
    let gpu_context = wglymr_runtime::gpu::GpuContext::new(instance, adapter, device, queue);

    EditorRuntime::with(|rt| {
        if let Err(e) = rt.init_gpu(gpu_context) {
            wglymr_runtime::logging::error(&format!("Failed to initialize GPU: {}", e));
        }
    });
...
pub fn create_view(id: &str) {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.create_view(id) {
            wglymr_runtime::logging::error(&format!("Failed to create view: {}", e));
        }
    });
}
...
pub fn destroy_view(id: &str) {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.destroy_view(id) {
            wglymr_runtime::logging::error(&format!("Failed to destroy view: {}", e));
        }
    });
}
...
pub fn attach_view(
    ...
    EditorRuntime::with(|rt| {
        let gpu = match rt.gpu() {
            Some(gpu) => gpu,
            None => {
                wglymr_runtime::logging::error("GPU not initialized - call init_gpu first");
                return;
            }
        };
            ...
            .create_surface(wgpu::SurfaceTarget::Canvas(canvas))
        {
            Ok(surf) => surf,
            Err(e) => {
                wglymr_runtime::logging::error(&format!("Failed to create surface: {:?}", e));
                return;
            }
        };
        ...
        if let Err(e) = rt.attach_view(id, surface_handle, css_width, css_height, backing_scale) {
            wglymr_runtime::logging::error(&format!("Failed to attach view: {}", e));
        }
    });
}
...
pub fn detach_view(id: &str) {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.detach_view(id) {
            wglymr_runtime::logging::error(&format!("Failed to detach view: {}", e));
        }
    });
}
...
pub fn resize_view(id: &str, css_width: u32, css_height: u32, backing_scale: f32) {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.resize_view(id, css_width, css_height, backing_scale) {
            wglymr_runtime::logging::error(&format!("Failed to resize view: {}", e));
        }
    });
}
...
pub fn set_view_camera(id: &str, x: f32, y: f32, zoom: f32) {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.set_view_camera(id, x, y, zoom) {
            wglymr_runtime::logging::error(&format!("Failed to set view camera: {}", e));
        }
    });
}
...
pub fn request_render(id: &str) {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.request_render(id) {
            wglymr_runtime::logging::error(&format!("Failed to request render: {}", e));
        }
    });
}
...
pub fn set_visible(id: &str, visible: bool) {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.set_visible(id, visible) {
            wglymr_runtime::logging::error(&format!("Failed to set visibility: {}", e));
        }
    });
}
...
pub fn render() {
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.render_dirty_views() {
            wglymr_runtime::logging::error(&format!("Failed to render: {}", e));
        }
    });
}
...
pub fn handle_mouse_move(
    ...
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.handle_mouse_move(id, screen_x, screen_y, shift, ctrl, alt) {
            wglymr_runtime::logging::error(&format!("Failed to handle mouse move: {}", e));
        }
    });
}
...
pub fn handle_mouse_down(
    ...
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.handle_mouse_down(id, screen_x, screen_y, button, shift, ctrl, alt) {
            wglymr_runtime::logging::error(&format!("Failed to handle mouse down: {}", e));
        }
    });
}
...
pub fn handle_mouse_up(
    ...
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.handle_mouse_up(id, screen_x, screen_y, button, shift, ctrl, alt) {
            wglymr_runtime::logging::error(&format!("Failed to handle mouse up: {}", e));
        }
    });
}
...
pub fn handle_mouse_enter(
    ...
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.handle_mouse_enter(id, screen_x, screen_y, button, shift, ctrl, alt) {
            wglymr_runtime::logging::error(&format!("Failed to handle mouse enter: {}", e));
        }
    });
}
...
pub fn handle_mouse_leave(
    ...
    EditorRuntime::with(|rt| {
        if let Err(e) = rt.handle_mouse_leave(id, screen_x, screen_y, button, shift, ctrl, alt) {
            wglymr_runtime::logging::error(&format!("Failed to handle mouse leave: {}", e));
        }
    });
}
...
pub fn dispatch_command(json: &str) {
    use wglymr_runtime::commands::{dispatch, Command};
    // wglymr_runtime::logging::debug(&format!("Received json command: {:?}", json));

    let command: Command = match serde_json::from_str(json) {
        Ok(cmd) => cmd,
        Err(e) => {
            wglymr_runtime::logging::error(&format!("Failed to deserialize command: {}", e));
            return;
        }
    };
    ...
    EditorRuntime::with(|rt| {
        if let Err(e) = dispatch(rt, command) {
            wglymr_runtime::logging::error(&format!("Command execution failed: {}", e));
        }
    });
}
