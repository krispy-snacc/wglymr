...
export function PanelShell({ editorType, onEditorTypeChange, viewId }: PanelShellProps) {
    ...
    return (
        <div className="h-full flex flex-col bg-zinc-950">
            <div className="flex items-center gap-1 px-2 bg-white/[0.02] border-b border-white/[0.06] shrink-0" style={{ height: "28px" }}>
                <Select.Root value={editorType} onValueChange={(value) => onEditorTypeChange(value as EditorType)}>
                    ...
                    <Select.Portal>
                        <Select.Content
                            className="bg-zinc-900/98 backdrop-blur-sm border border-white/[0.12] rounded-md shadow-2xl overflow-hidden"
                            style={{ zIndex: 9999 }}
                            position="popper"
                            sideOffset={2}
                        >
